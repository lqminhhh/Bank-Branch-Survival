...
We will now test a more advanced model, an improved version of decision tree by adding randomness.
...
```{r}
which.min(model_boosting$cv.error)
```
...
predictions <- predict(xgb_cv, newdata = test_data)
confusionMatrix(predictions, test_data$closed, positive = "Closed")
```
...
# Evaluate
confusionMatrix(factor(pred_labels), factor(y_test))
```
